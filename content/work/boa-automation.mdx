...
I proposed building an automation suite and taught myself Google Apps Script from scratch to do it. The organisation used Google Workspace for everything, so building on that ecosystem meant zero additional infrastructure cost and no migration required.
...
- **Offer letter generation** — Populates template documents with student data and sends them automatically when an application is approved
...
- Administrative team reduced from 4 staff to 2 part-time — approximately **€74,000 in annual savings**
- Payment reconciliation system with automated follow-ups achieved a **20% increase in payment collection**
...
- Code is proprietary (internal business tools)
