...
tags: ["Next.js", "TypeScript", "Tailwind CSS", "Anthropic", "Framer Motion"]
status: "shipped"
fun: true
stats:
  - value: "95+"
    label: "Lighthouse"
  - value: "Edge"
    label: "Streaming chatbot"
  - value: "OffscreenCanvas"
    label: "Off-main-thread bg"
---

<Demo
  github="https://github.com/mojoro/john-moorman-portfolio"
  caption="You're using this site right now. Try the chatbot in the corner. The animated circuit board behind this text runs entirely off the main thread."
/>
...
- [`app/api/chat/route.ts`](https://github.com/mojoro/john-moorman-portfolio/blob/main/app/api/chat/route.ts) — the streaming chatbot route, with all seven cost-protection layers wired in
- [`workers/circuit-worker.ts`](https://github.com/mojoro/john-moorman-portfolio/blob/main/workers/circuit-worker.ts) — generation, rendering, and animation of the PCB background, all off the main thread via OffscreenCanvas
- [`lib/chatbot-prompt.ts`](https://github.com/mojoro/john-moorman-portfolio/blob/main/lib/chatbot-prompt.ts) — the system prompt itself, the single most important piece of writing on this site
