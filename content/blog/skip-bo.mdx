...
(Update: the AWS free tier has since expired and the box is gone, so the hosted game is offline for now. Everything below describes what shipped and still runs from the repo.)
...
The free AWS tier has expired, so the box is shut down and there's no live game to point you at right now. I'm not paying AWS prices for a card game, so the plan is to redeploy somewhere cheaper (probably [fly.io](https://fly.io)) when I get to it. The deploy is two idempotent scripts, so bringing it back up is an afternoon, not a project. When it's back, open a room, send me the code, and I'll be there.

## Links

[GitHub](https://github.com/mojoro/skip-bo)
