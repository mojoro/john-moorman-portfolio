...
URL or topic
  ...
  → LLM             Generate monologue or HOST_A / HOST_B dialogue script
...
The LLM cascade order is also configurable from the same panel. Drag and drop to reorder. The order you set persists as part of the profile.

## A Note on the Qwen3-TTS Integration

At the end of the week I added a second local TTS option alongside pocket-tts: Qwen3-TTS, a recent model from Alibaba with nine named speakers and ten language options. It lives in `qwen-tts-server/` as a separate sidecar with the same API contract as the pocket-tts server, and it slots into the TTS router as a fourth backend: LOCAL / QWEN3 / 11LABS / OPENAI.
