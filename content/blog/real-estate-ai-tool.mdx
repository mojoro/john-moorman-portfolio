...
![n8n pipeline workflow diagram](/images/blog/real-estate-ai-tool/pipeline-diagram.png)
...
![Daily email digest with recommended listings](/images/blog/real-estate-ai-tool/email-digest.png)
...
![Client-facing dashboard with filterable listing table](/images/blog/real-estate-ai-tool/dashboard.png)

The Next.js frontend reads from Airtable and presents listings in a filterable table with the AI status prominently displayed. The client can sort by yield, filter by status, and access all data on individual listings to see the full analysis and data collected. Crucially, the dashboard connects directly to Airtable for real-time editing: the client can annotate listings, update statuses, and add notes, and changes write back immediately.

I built the dashboard using Claude Code with two MCP (Model Context Protocol) servers: one for filesystem operations and one for interacting with the Airtable API during development. This workflow let me iterate on the data layer rapidly without manual API testing, and it's a good example of how AI-native development tools can compress what would otherwise be a multi-day integration task.

Keeping Airtable as the data layer rather than a custom database was a deliberate tradeoff: it meant faster development and gave the client a familiar interface to inspect the raw data directly if they ever needed to.
