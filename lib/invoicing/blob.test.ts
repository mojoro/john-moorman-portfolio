...
const localPath = join(process.cwd(), "public", "invoices", `${invoiceNo}.pdf`)
...
describe("invoice PDF storage", () => {
  it("falls back to public local storage in development without a Vercel Blob token", async () => {
    ...
    try {
      ...
      assert.equal(result.url, `/invoices/${invoiceNo}.pdf`)
